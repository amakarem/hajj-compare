<?php
$template_name = 'material-design-2';
...
// -- START -- TEMPLATE HTML
	...
	foreach ( $ads as $key => $ad ) {
		...
		echo '<div class="bsaProItem '.(($key % $col_per_row == 0) ? "bsaReset" : "").' card" data-animation="'.bsa_space($sid, "animation").'" style="'.((bsa_space($sid, "animation") == "none" OR bsa_space($sid, "animation") == NULL) ? "opacity:1" : "").'">'; // -- START -- ITEM
		...
		echo '<div class="bsaProItemInner__copy z-depth-2" style="background-color:'.bsa_space($sid, 'ad_bg').'">'; // -- START -- ITEM COPY

			echo '<div class="bsaProItemInner__copyInner" style="background-color:'.bsa_space($sid, 'ad_bg').'">'; // -- START -- ITEM COPY INNER
