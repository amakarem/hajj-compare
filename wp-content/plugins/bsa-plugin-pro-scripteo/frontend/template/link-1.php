...
if ( isset($_POST['bsa_get_required_inputs']) ) {
	...
	return 'title,url'; // inputs shows in form (default: 'title,desc,url,img or html')
...
// -- START -- TEMPLATE HTML
	...
	echo '<div class="bsaProItems '.bsa_space($sid, "grid_system").' '.((strpos(bsa_space($sid, 'display_type'), 'carousel') !== false) ? 'bsa-owl-carousel bsa-owl-carousel-'.$sid : '').'" style="background-color:'.bsa_space($sid, 'ads_bg').'; border-color:'.bsa_space($sid, 'ad_extra_color_1').'">'; // -- START -- ITEMS
	...
	foreach ( $ads as $key => $ad ) {
		...
		echo '<p class="bsaProItemInner__title" style="color:'.bsa_space($sid, 'ad_title_color').'">'.$ad['title'].'</p>'; // -- ITEM -- TITLE
